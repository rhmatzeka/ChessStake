...
const schema = z.object({
  ...
  amountWei: z.string().regex(/^\d{1,40}$/, 'Amount must be a positive wei string.'),
});
...
export async function POST(request: Request, { params }: { params: Promise<{ gameId: string }> }) {
  try {
    ...
    const status = ['GAME_NOT_ACTIVE', 'TURN_LOCKED', 'WRONG_TEAM_TURN', 'TEAM_ALREADY_LOCKED', 'ALREADY_BET_THIS_TURN', 'PIECE_HAS_NO_LEGAL_MOVE', 'INVALID_PIECE_PRICE'].includes(message) ? 400 : 500;
